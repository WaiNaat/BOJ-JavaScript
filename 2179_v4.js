...
v3이랑 방법은 같은데 Map 대신 Object 사용해봄
Map이 조금 더 빠른듯?
*/
// class
class Pair {
...
for (let i = 0; i < N - 1; i += 1) {
  ...
  if (prefixPairs[prefix] === undefined) {
    prefixPairs[prefix] = new Pair(prefix.length, ...words[i], ...words[i + 1]);
  } else {
    const pair = prefixPairs[prefix];
    pair.update(...words[i]);
...
let sol = new Pair(-1);
prefixPairs = Object.values(prefixPairs);
prefixPairs.forEach((pair) => { if (pair.hasMorePriority(sol)) sol = pair; });
