...
class Queue
{
    ...
    dequeue()
    {
        if (this.isEmpty()) return new Tree(-1, -1, -1);
        const del = this.first.tree;
        this.length--;
        if (this.isEmpty())
        {
            this.first = null;
...
for (let i=N+1; i<N+1+M; i++)
{
    ...
    r--;
    c--;
    land[r][c].enqueue(new Tree(r, c, a));
}
...
for (let year=0; year<K; year++)
{
    ...
    for (let r=0; r<N; r++)
    {
        for (let c=0; c<N; c++)
        {
            ...
            let propagate_cnt = 0;
            let died = 0;
            for (let cur = land[r][c].last; 
                cur != null && cur.tree.age <= nutrient[r][c];
                cur = cur.prev)
            {
                nutrient[r][c] -= cur.tree.age;
                cur.tree.age++;
                cnt++;
            ...
            if (cnt < land[r][c].length)
            {
                while (land[r][c].length > cnt)
                {
                    ...
                    died += Math.floor(died_tree.age / 2);
                    total_trees--;
                }
            }
    ...
    for (let [r, c, propagate_cnt] of propagates)
    {
        for (let [dr, dc] of directions)
        {
            ...
            for (let i=0; i<propagate_cnt; i++)
                land[r2][c2].enqueue(new Tree(r2, c2, 1));
        }
    }
}
